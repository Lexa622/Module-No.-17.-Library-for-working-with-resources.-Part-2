...
async def tasks_by_user_id(db: Annotated[Session, Depends(get_db)], user_id: int):
    ...
    if tasks is None:   # бесполезная запись
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tasks was not found")
    return tasks


@router.post("/create")
async def create_user(db: Annotated[Session, Depends(get_db)], cr_user: CreateUser):
    db.execute(insert(User).values(username=cr_user.username,
...
@router.put("/update")
async def update_user(db: Annotated[Session, Depends(get_db)], user_id: int, updt_user: UpdateUser):
